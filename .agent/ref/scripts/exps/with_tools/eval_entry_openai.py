#!/usr/bin/env python3
"""
eval_entry_openai.py — 使用 OpenAI-兼容 API (本地 vLLM server) 的评测入口

与 eval_entry.py 的区别:
  1. 使用 model: openai (OpenaiModels) 连接本地 vLLM API server
  2. 不需要 accelerate / GPU（GPU 由 vLLM server 独占）
  3. 计时 hook 针对 OpenaiModels.generate 打补丁
  4. 不需要 model preflight 检查
"""
import argparse
import importlib
import json
...
# ============================================================
# 计时 hook — 针对 OpenaiModels.generate
# ============================================================

def install_generation_timer(timing):
    """Patch OpenaiModels.generate to record per-sample wall-clock time."""
    from tool_server.tf_eval.models.openai import OpenaiModels

    original_generate = OpenaiModels.generate
    ...
    OpenaiModels.generate = timed_generate


# ============================================================
# 结果元数据注入 (与 eval_entry.py 一致)
# ============================================================
...
# ============================================================
# 任务覆盖 (与 eval_entry.py 一致的本地数据加载)
# ============================================================
...
def main():
    ...
    timing = {
        ...
        "vllm_base_url": os.environ.get("VLLM_BASE_URL", "unknown"),
        "lm_request_concurrency": None,
        "tool_concurrency": int(os.environ.get("TF_EVAL_TOOL_CONCURRENCY", "1")),
    }
    ...
    try:
        ...
        timing["lm_request_concurrency"] = int(
            model_args.model_args.get("request_concurrency", model_args.batch_size)
        )
        timing["eval_batch_size"] = int(model_args.batch_size)

        # Import task modules before allocating GPU memory so dependency errors fail fast.
        for task_name in task_args.task_name:
            evaluator_module.get_task_functions(task_name)

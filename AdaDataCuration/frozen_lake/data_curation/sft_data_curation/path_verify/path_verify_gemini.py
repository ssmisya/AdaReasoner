...
from frozen_lake.data_curation.sft_data_curation.prompts import TOOL_PROMPTS, PATH_VERIFY_TASK_INSTRUCTION_SHORT, REASONING_REPHRASE_FINAL_PROMPT
...
class GeminiVerifyGenerator:
    ...
    def load_data(self):
        ...
        # 过滤出有随机路径的条目
        if self.args.max_samples < len(self.valid_dataset):
            self.valid_dataset = self.valid_dataset[:self.args.max_samples]
    ...
    def generate_dataset(self):
        ...
        sharegpt_output = os.path.join(self.output_path, "verify_sharegpt_data.json")
...
def main():
    ...
    parser.add_argument('--input_path', type=str, default="/mnt/petrelfs/songmingyang/code/reasoning/tool_util_code/frozen_lake/data_curation/frozen_lake_metadata_v2/metadata_split/path_verify/sft.jsonl", 
                        help='输入数据集路径')
    parser.add_argument('--output_path', type=str, default="/mnt/petrelfs/songmingyang/code/reasoning/tool_util_code/frozen_lake/data_curation/frozen_lake_metadata_v2/path_verify", 
                        help='输出SFT数据集目录')

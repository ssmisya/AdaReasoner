...
class DataProto:
    ...
    def concat(data: List["DataProto"]) -> "DataProto":
        ...
        The meta_info is aggregated properly, especially for tool_stats.
        ...
        # 添加调试信息，找出哪个键的数组维度不匹配
        for key, val in non_tensor_batch.items():
            try:
                non_tensor_batch[key] = np.concatenate(val, axis=0)
            except ValueError as e:
                # 打印详细的错误信息
                print(f"Error concatenating key: {key}")
                for i, arr in enumerate(val):
                    print(f"  Array {i} shape: {arr.shape}, dtype: {arr.dtype}")
                raise ValueError(f"Error concatenating arrays for key '{key}': {e}")

        # 聚合meta_info，特别处理tool_stats
        aggregated_meta_info = {}
        if data and data[0].meta_info:
            # 复制第一个meta_info作为基础
            aggregated_meta_info = data[0].meta_info.copy()
            
            # 特殊处理tool_stats字段
            if 'tool_stats' in aggregated_meta_info:
                all_tool_stats = []
                for d in data:
                    if d.meta_info and 'tool_stats' in d.meta_info:
                        all_tool_stats.append(d.meta_info['tool_stats'])
                ...
                if all_tool_stats:
                    ...
                    for gpu_stats in all_tool_stats:
                        ...
                        for key, value in gpu_stats.items():
                            if key not in aggregated_tool_stats:
                                aggregated_tool_stats[key] = 0
                            aggregated_tool_stats[key] += value
                    
                    # 重新计算成功率
                    if 'tool_total_calls' in aggregated_tool_stats and aggregated_tool_stats['tool_total_calls'] > 0:
                        aggregated_tool_stats['tool_success_rate'] = aggregated_tool_stats.get('tool_successful_calls', 0) / aggregated_tool_stats['tool_total_calls']
                    
                    # 重新计算各工具的成功率
                    for key in list(aggregated_tool_stats.keys()):
                        if key.endswith('_calls') and not key.endswith('_total_calls') and not key.endswith('_successful_calls'):
                            ...
                            if success_key in aggregated_tool_stats:
                                calls = aggregated_tool_stats[key]
                                success = aggregated_tool_stats[success_key]
                                aggregated_tool_stats[rate_key] = success / max(calls, 1)
                    ...
                    aggregated_meta_info['tool_stats'] = aggregated_tool_stats
                    print(f"[DEBUG] Aggregated tool_stats in DataProto.concat: {aggregated_tool_stats}")

        return DataProto(batch=new_batch, non_tensor_batch=non_tensor_batch, meta_info=aggregated_meta_info)

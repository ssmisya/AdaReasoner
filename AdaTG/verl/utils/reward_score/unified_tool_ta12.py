...
openai_api_url = os.environ.get("JUDGE_BASE_URL", "http://SH-IDC1-10-140-37-132:16113/v1"),
...
def compute_score(data_sources, solution_strs, ground_truths, extra_infos=None, tool_rewards=None, **kwargs):
    ...
    for i in range(len(data_sources)):
        ...
        if format_reward:
            ...
            # 计算总分
            if accuracy_reward:
                total_score = 3  # 答对了给8分
            else:
                total_score = accuracy_reward / 4 * 2 + processed_tool_reward / 4  # 答错了，按照工具使用情况给分

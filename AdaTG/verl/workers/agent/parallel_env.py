...
def substitute_system_prompt(prompt_str, new_system_prompt,):
    system_prompt_pattern = r"<\|im_start\|>(.*?)<\|im_end\|>"
    if re.search(system_prompt_pattern, prompt_str, flags=re.DOTALL):
        # modified_prompt = re.sub(system_prompt_pattern, f"<|im_start|>system\n\n{new_system_prompt}<|im_end|>", prompt_str,count=1, flags=re.DOTALL)
...
def agent_rollout_loop(config, vllm_engine, vllm_inputs, prompts, multi_modal_inputs, sampling_params):
    ...
    if "randomized_to_original" in prompts.non_tensor_batch:
        randomized_to_original_dicts = prompts.non_tensor_batch["randomized_to_original"]
    else:
        randomized_to_original_dicts = [{}] * batch_size
        print(f"[DEBUG] No randomized_to_original found in prompts.non_tensor_batch")   
    
    
    env = ParallelEnv(config.agent, tokenizer, processor)
    tool_manager = env.tool_manager
    
    
    # new_system_prompt = tool_manager.get_tool_prompt()
    
    # for prompt_idx in range(len(vllm_inputs)):
    #     current_prompt_token_ids = vllm_inputs[prompt_idx]['prompt_token_ids']
    #     current_prompt = tokenizer.decode(current_prompt_token_ids)
    #     new_current_prompt = substitute_system_prompt(current_prompt, new_system_prompt)
    #     vllm_inputs[prompt_idx]['prompt_token_ids'] = tokenizer.encode(new_current_prompt)
        
    #     current_batch = prompts[prompt_idx].batch
    #     current_input_ids = current_batch['input_ids']
    #     max_length = current_input_ids.shape[-1]
        
    #     current_input_ids_text = tokenizer.decode(current_input_ids)
    #     new_current_input_ids_text = substitute_system_prompt(current_input_ids_text, new_system_prompt)
    #     new_input_ids, new_attention_masks = tokenizer(
    #         new_current_input_ids_text,
    #         return_tensors='pt',
    ...
    #     new_input_ids = new_input_ids[0].to(current_input_ids.device)
    #     new_attention_masks = new_attention_masks[0].to(current_input_ids.device)
    #     pads_num = new_current_input_ids_text.count(tokenizer.pad_token)
    #     new_attention_masks[:pads_num] = 0  # 保持padding部分的attention_mask为0
        
    #     prompts[prompt_idx].batch['input_ids'] = new_input_ids
    #     prompts[prompt_idx].batch['attention_mask'] = new_attention_masks
        
    #     prompts[prompt_idx].non_tensor_batch["raw_prompt"][0]["content"][0]["text"] = new_system_prompt
    ...
    print("[TP DEBUG]", "pid", os.getpid(), "tp_rank", pg.rank, "tp_world", pg.world_size, "is_first", pg.is_first_rank)
    ...
    for step in range(config.agent.max_turns):
        ...
        action_mask = pg.broadcast_object(active_mask)
        ...
        # active_data_sources = []
        # active_randomized_to_original_dicts = []
        ...
        #     if original_idx < len(randomized_to_original_dicts):
        #         active_randomized_to_original_dicts.append(randomized_to_original_dicts[original_idx])
        #     else:
        #         active_randomized_to_original_dicts.append(None)
                
        # start_time = time.time()
        # obs_results = env.step(active_indices, actions, active_data_sources, active_randomized_to_original_dicts)
        # end_time = time.time()
...
class ParallelEnv:
    ...
    def __init__(self, env_config, tokenizer, processor, **kwargs):
        ...
        self.randomize_tool_name = self.config.tool_manager.randomize
        self.tool_manager = ToolManager(self.config.tool_manager.controller_addr, self.config.tool_manager.tools, randomize = self.randomize_tool_name)
    ...
    def call_tool_with_img(
        ...
        调用工具并处理重试
        """
        if randomized_to_original:
            original_to_randomized = {v: k for k, v in randomized_to_original.items()}
        else:
            original_to_randomized = None
        # 确保每次调用都被统计
        image_keys = self.tool_manager.origin_image_params
        if original_to_randomized:
            image_keys = [original_to_randomized.get(k, k) for k in image_keys]
        ...
        try:
            # 直接调用工具，不设置超时
            tool_result = self.tool_manager.call_tool(api_name, api_params, randomized_to_original)
    ...
    def _process_single_action(self, action_data):
        ...
        randomized_to_original = action_data.get('randomized_to_original', None)
        result = {
            'index': i,
            'obs': {},
            'tool_reward': 0.0,
        ...
        try:
            ...
            if tool_calls is not None and len(tool_calls) > 0:
                ...
                if randomized_to_original is not None:
                    real_tool_name = randomized_to_original.get(tool_name, tool_name)
                else:
                    real_tool_name = tool_name
                
                # 工具名称白名单检查
                tool_name_list = self.available_tools
                if real_tool_name not in tool_name_list:
                    # 工具不在白名单中，直接记录为失败的unrecognized_tool调用
                    result['tool_stats']['tool_calls'].append({
                        'tool_name': real_tool_name,  # 保留原始工具名，让统计类处理归类
                        'success': False,
                        'data_source': data_source
                    })
                    
                    error_text = f"Tool {tool_name} ({real_tool_name}) is not available."
                ...
                # breakpoint()
                # 调用工具，传入idx用于访问图像历史
                tool_result = self.call_tool_with_img(tool_name, tool_params, idx, randomized_to_original = randomized_to_original)
                
                # 添加详细的调试信息
                # print(f"[DEBUG] Tool call result for {tool_name}: status={tool_result.get('status', 'unknown')}, error_code={tool_result.get('error_code', 'unknown')}")
                ...
                if tool_result:
                    ...
                    if not success:
                        print(f"[DEBUG] Tool {tool_name} ({real_tool_name}) call failed: status={status}, error_code={error_code}, message={tool_result.get('message', '填充值unknown')}")
    ...
    def step(self, active_indices, actions, data_sources=None, randomized_to_original_dicts = None):
        """
        Input:
        ...
        for i, idx, action_text in zip(real_indices, valid_indices, valid_actions):
            ...
            randomized_to_original = None
            if randomized_to_original_dicts is not None and i < len(randomized_to_original_dicts):
                randomized_to_original = randomized_to_original_dicts[i]
            ...
            action_data_list.append({
                ...
                'data_source': data_source,
                'randomized_to_original': randomized_to_original,
            })
        # breakpoint()
        # 使用线程池并发执行工具调用

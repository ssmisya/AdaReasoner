    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            if self.config.agent.activate_agent:
                ...
                # 不要将tool_stats放入non_tensor_batch，因为它是字典类型
            ...
            if self.sampling_params.n > 1 and do_sample:
                ...
                # 扩展所有非张量数据以匹配新的batch_size
                for key, value in list(non_tensor_batch.items()):
                    if key not in ["raw_prompt_ids", "multi_modal_data", "origin_multi_modal_data"]:
        ...
        final_meta_info = {}
        if self.config.agent.activate_agent and hasattr(agent_proto, 'meta_info') and agent_proto.meta_info:
            # 将agent_proto的meta_info合并到最终结果中
            final_meta_info = agent_proto.meta_info.copy()
        ...
        if self.config.agent.activate_agent:
            ...
            print(f' [DEBUG agent output proto] batch.keys()= {batch.keys()},non_tensor_batch.keys()= {non_tensor_batch.keys()}')
        ...
        # 创建DataProto时包含meta_info
        return DataProto(batch=batch, non_tensor_batch=non_tensor_batch, meta_info=final_meta_info)

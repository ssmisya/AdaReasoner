...
COPY ./AdaReasoner /app/AdaReasoner
...
RUN pip install -e /app/AdaReasoner

RUN pip install -r /app/AdaReasoner/requirements/tool_server_requirements.txt
...
# === 设置工作目录 ===
WORKDIR /app/AdaReasoner

# === 设置默认启动命令（等价于 %runscript）===
CMD ["python", "/app/AdaReasoner/tool_server/tool_workers/scripts/launch_scripts/start_server_local.py", \
     "--config",  \
     "/app/AdaReasoner/tool_server/tool_workers/scripts/launch_scripts/config/service_apptainer.yaml"]

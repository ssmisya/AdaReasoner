...
session_name="tool-factory111-$(date +%Y%m%d_%H%M%S)"
...
# 运行python服务
tmux send-keys -t $session_name "python -m tool_server.tool_workers.scripts.launch_scripts.start_server_config_shy_指定结点1 --config tool_server/tool_workers/scripts/launch_scripts/config/all_service_example_shy_指定结点1.yaml" C-m

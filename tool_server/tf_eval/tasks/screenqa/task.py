...
def load_data_function():
    ...
    for idx,item in enumerate(dataset):
        # item_id = f"screenqa_{idx}"
        item_id = f"screenqa_{item['id']}"
        image = item["image"]
        text = item["question"]
    ...
    print(f"DEBUG: webquest数据加载完成，总数: {len(meta_data)}")
    if len(meta_data) > 0:
        print(f"DEBUG: 第一个数据项idx: {meta_data[0]['idx']}")
        print(f"DEBUG: 最后一个数据项idx: {meta_data[-1]['idx']}")
    return meta_data

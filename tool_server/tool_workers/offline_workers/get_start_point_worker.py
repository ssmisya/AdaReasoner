...
logger = build_logger("get_start_point_worker")

class GetStartPoint(BaseOfflineWorker):
    """
    获取起始点位置的工具
    通过识别图像中的特定对象来确定起始点坐标
    """
    
    def __init__(self):
        super().__init__(model_name="GetStartPoint")
        self.instruction = {
            "type": "function",
            "function": {
                "name": self.model_name,
                "description": "Identify the starting point location in the maze image by detecting the starting position marker.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "image": {
    ...
    def _execute(self, params):
        ...
        try:
            ...
            logger.info("Calling Point tool to locate Elf...")
            ...
            if point_result.get("status") != "success":
                ...
                return {
                    "status": "failed",
                    "message": f"Failed to locate starting point: {error_msg}",
            ...
            if not points or len(points) == 0:
                return {
                    "status": "failed",
                    "message": "No starting point detected in the image",
            ...
            result = {
                ...
                "message": f"Starting point located at {start_point}",
                "error_code": SUCCESS
            }
            ...
            logger.error(f"Error detecting start point: {str(e)}")
            return {
                "status": "failed",
                "message": f"Error detecting start point: {str(e)}",
                "error_code": TOOL_RUN_FAILED
            }

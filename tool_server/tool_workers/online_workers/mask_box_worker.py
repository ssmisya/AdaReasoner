...
logger = build_logger(__file__, f"mask_box_worker_{worker_id}.log")
...
class MaskBoxWorker(BaseToolWorker):
    def __init__(self, worker_arguments: MaskBoxArguments = None):
        # 在调用父类初始化前先设置模型名称
    ...
    def generate(self, params):
        """执行遮罩框操作并返回结果"""
        return self._process_mask_box_request(params)
        
    def _process_mask_box_request(self, params):
        """分离出实际处理逻辑，方便并发调用"""
        tool_reward = 2.0
...
if __name__ == "__main__":
    parser = HfArgumentParser((MaskBoxArguments,))

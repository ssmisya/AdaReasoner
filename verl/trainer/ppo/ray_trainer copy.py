        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if self.config.actor_rollout_ref.rollout.agent.activate_agent:
                    ...
                    tool_metrics = compute_tool_metrics(batch=batch)  # 新增工具指标
...
def compute_tool_metrics(batch):
    """
    计算工具调用相关的指标
    ...
    if hasattr(batch, 'meta_info') and batch.meta_info and 'tool_stats' in batch.meta_info:
        tool_stats = batch.meta_info['tool_stats']
        
        # 总工具调用次数
        tool_metrics['tool_calls/total_calls'] = tool_stats.get('total_calls', 0)
        
        # 工具调用成功次数
        tool_metrics['tool_calls/successful_calls'] = tool_stats.get('successful_calls', 0)
        
        # 工具调用成功率
        total_calls = tool_stats.get('total_calls', 0)
        if total_calls > 0:
            success_rate = tool_stats.get('successful_calls', 0) / total_calls
            tool_metrics['tool_calls/success_rate'] = success_rate
        else:
            tool_metrics['tool_calls/success_rate'] = 0.0
        
        # 平均工具调用次数（假设batch_size可以从其他地方获取）
        if hasattr(batch, 'batch') and len(batch.batch) > 0:
            batch_size = batch.batch.shape[0]
            tool_metrics['tool_calls/avg_calls_per_sample'] = total_calls / batch_size if batch_size > 0 else 0.0
        
        # 各个工具的调用次数
        tool_calls = tool_stats.get('tool_calls', {})
        for tool_name, count in tool_calls.items():
            tool_metrics[f'tool_calls/{tool_name}_calls'] = count
        
        # 各个工具的成功次数
        tool_success = tool_stats.get('tool_success', {})
        for tool_name, count in tool_success.items():
            tool_metrics[f'tool_calls/{tool_name}_success'] = count
            
            # 各个工具的成功率
            total_tool_calls = tool_calls.get(tool_name, 0)
            if total_tool_calls > 0:
                success_rate = count / total_tool_calls
                tool_metrics[f'tool_calls/{tool_name}_success_rate'] = success_rate
            else:
                tool_metrics[f'tool_calls/{tool_name}_success_rate'] = 0.0
